...
public class CVCT {
    ...
    public int get(IBitstream _F_bs) throws IOException {
        ...
        num_channels_in_section = _F_bs.getbits(8);
        for (cn=0; cn<num_channels_in_section; cn++) {
            channel = new CVCTChannel();
            _F_ret += channel.get(_F_bs);
            if (1==0) break;
        }
        reserved3 = _F_bs.getbits(6);
        if (reserved3 != 63) {
            Util.flerror("Const value mismatch for 'reserved3'");
            _F_ret++;
        }
        additional_descriptors_length = _F_bs.getbits(10);
        remain_desc_bytes = additional_descriptors_length;
    ...
    public int put(IBitstream _F_bs) throws IOException {
        ...
        _F_bs.putbits(num_channels_in_section, 8);
        for (cn=0; cn<num_channels_in_section; cn++) {
            _F_ret += channel.put(_F_bs);
            if (1==0) break;
        }
        _F_parse = 6;
        reserved3 = 63;
        _F_bs.putbits(reserved3, _F_parse);
        _F_bs.putbits(additional_descriptors_length, 10);
        remain_desc_bytes = additional_descriptors_length;
        while (remain_desc_bytes>0) {
    ...
    public int putxml(IBitstream _F_bs, boolean bAttr) throws IOException {
        ...
        _F_parse = 8;
        num_channels_in_section = _F_bs.getbits(_F_parse);
        if (bAttr) {
            XML.WriteXmlElement("<num_channels_in_section type=\"flUInt\" bitLen=\"" + _F_parse + "\">" + num_channels_in_section + "</num_channels_in_section>");
        }
        else {
            XML.WriteXmlElement("<num_channels_in_section bitLen=\"" + _F_parse + "\">" + num_channels_in_section + "</num_channels_in_section>");
        }
        for (cn=0; cn<num_channels_in_section; cn++) {
            channel = new CVCTChannel();
            XML.IntoAClass("channel", 0);
            _F_ret += channel.putxml(_F_bs, bAttr);
        ...
        _F_parse = 10;
        additional_descriptors_length = _F_bs.getbits(_F_parse);
        if (bAttr) {
            XML.WriteXmlElement("<additional_descriptors_length type=\"flUInt\" bitLen=\"" + _F_parse + "\">" + additional_descriptors_length + "</additional_descriptors_length>");
        }
        else {
            XML.WriteXmlElement("<additional_descriptors_length bitLen=\"" + _F_parse + "\">" + additional_descriptors_length + "</additional_descriptors_length>");
        }
        remain_desc_bytes = additional_descriptors_length;
        while (remain_desc_bytes>0) {
            descriptor = new Descriptor();
