...
public class CVCTChannel {
    ...
    public int get(IBitstream _F_bs) throws IOException {
        ...
        for (_F_dim0 = 0; _F_dim0 < _F_dim0_end; _F_dim0++) {
            short_name[_F_dim0] = _F_bs.getbits(16);
        }
        reserved1 = _F_bs.getbits(4);
        if (reserved1 != 15) {
            Util.flerror("Const value mismatch for 'reserved1'");
            _F_ret++;
        }
        major_channel_number = _F_bs.getbits(10);
        minor_channel_number = _F_bs.getbits(10);
        modulation_mode = _F_bs.getbits(8);
        carrier_frequency = _F_bs.getbits(32);
        channel_TSID = _F_bs.getbits(16);
        program_number = _F_bs.getbits(16);
        ETM_location = _F_bs.getbits(2);
        access_controlled = _F_bs.getbits(1);
        hidden = _F_bs.getbits(1);
        path_select = _F_bs.getbits(1);
        out_of_band = _F_bs.getbits(1);
        hide_guide = _F_bs.getbits(1);
        reserved3 = _F_bs.getbits(3);
        if (reserved3 != 7) {
            Util.flerror("Const value mismatch for 'reserved3'");
            _F_ret++;
        }
        service_type = _F_bs.getbits(6);
        source_id = _F_bs.getbits(16);
