...
public class TVCT {
    ...
    public int get(IBitstream _F_bs) throws IOException {
        ...
        ETT_table_id_extension = _F_bs.getbits(16);
        if (ETT_table_id_extension != 0) {
        ...
        ETM_id = _F_bs.getbits(32);
        extended_text_message = new MultiString();
        _F_ret += extended_text_message.get(_F_bs);
    ...
    public int put(IBitstream _F_bs) throws IOException {
        ...
        _F_bs.putbits(ETT_table_id_extension, _F_parse);
        _F_parse = 2;
        reserved2 = 3;
        ...
        _F_bs.putbits(ETM_id, 32);
        _F_ret += extended_text_message.put(_F_bs);
    ...
    public int putxml(IBitstream _F_bs, boolean bAttr) throws IOException {
        ...
        _F_parse = 16;
        ETT_table_id_extension = _F_bs.getbits(_F_parse);
        if (ETT_table_id_extension != 0) {
            Util.flerror("Const value mismatch for 'ETT_table_id_extension'");
            _F_ret++;
            if (bAttr) {
                XML.WriteXmlElement("<ETT_table_id_extension type=\"flUInt\" bitLen=\"" + _F_parse + "\">" + ETT_table_id_extension + "</ETT_table_id_extension>");
            }
            else {
                XML.WriteXmlElement("<ETT_table_id_extension bitLen=\"" + _F_parse + "\">" + ETT_table_id_extension + "</ETT_table_id_extension>");
            }
        }
        else {
            if (bAttr) {
        ...
        _F_parse = 32;
        ETM_id = _F_bs.getbits(_F_parse);
        if (bAttr) {
            XML.WriteXmlElement("<ETM_id type=\"flUInt\" bitLen=\"" + _F_parse + "\">" + ETM_id + "</ETM_id>");
        }
        else {
            XML.WriteXmlElement("<ETM_id bitLen=\"" + _F_parse + "\">" + ETM_id + "</ETM_id>");
        }
        extended_text_message = new MultiString();
        XML.IntoAClass("extended_text_message", 0);
        _F_ret += extended_text_message.putxml(_F_bs, bAttr);
